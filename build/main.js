// elm-watch hot {"version":"1.1.2","targetName":"My target name","webSocketPort":55281}
"use strict";
(() => {
...
var $author$project$Main$MeasureFontRequested = {$: 'MeasureFontRequested'};
...
var $author$project$Main$HighlightModel = F3(
	function (mode, start, end) {
		return {end: end, mode: mode, start: start};
	});
var $author$project$Main$Scroll = F2(
	function (top, left) {
		return {left: left, top: top};
	});
var $author$project$Main$hqlExample = 'uid := userid |\n    groupBy(uid, function=[\n{statuscode = 200 | count(as=status200)},\n{statuscode = 500 | count(as=status500)}\n])\n| test(status500 > status200)\n| a or b and c\n';
var $author$project$Main$initHighlightModel = {end: 0, mode: $elm$core$Maybe$Nothing, start: 0};
...
var $author$project$Main$initModel = {
	autoComplete: false,
	cursor: {col: 0, pos: 0, row: 0, x: 0, y: 0},
	fontSize: {height: 10, width: 10},
	highlight: A3(
		$author$project$Main$HighlightModel,
		$elm$core$Maybe$Just($pablohirafuji$elm_syntax_highlight$SyntaxHighlight$Add),
		1,
		3),
	languageModel: $author$project$Main$initLanguageModel($author$project$Main$hqlExample),
...
var $author$project$Main$MeasureFontReceived = function (a) {
	return {$: 'MeasureFontReceived', a: a};
};
...
var $author$project$Main$cursor = F2(
	function (model, position) {
		...
		var col = function () {
			...
			if (_v0.b) {
				var hd = _v0.a;
				return $elm$core$String$length(hd) + 1;
			} else {
				return 0;
			}
		}();
...
var $author$project$Main$updateLangModel = F2(
	function (model, langModel) {
	...
	function (msg, model) {
		...
		switch (msg.$) {
			...
			case 'Move':
				var start = msg.a;
				var end = msg.b;
				var cursor_ = A2($author$project$Main$cursor, model, end);
				return _Utils_eq(start, end) ? _Utils_Tuple2(
					A2(
						$author$project$Main$updateLangModel,
						_Utils_update(
							model,
						...
						_Utils_update(
							languageModel,
							{
								highlight: {
									...
									mode: $elm$core$Maybe$Just($pablohirafuji$elm_syntax_highlight$SyntaxHighlight$Highlight),
									start: cursor_.row - 1
								}
							})),
					...
					A2(
						...
						_Utils_update(
							languageModel,
							{
								highlight: {end: 0, mode: $elm$core$Maybe$Nothing, start: 0}
							})),
					$elm$core$Platform$Cmd$none);
			case 'Frame':
				return function (a) {
			...
			case 'AutoCompleteToggled':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{autoComplete: !model.autoComplete}),
					$elm$core$Platform$Cmd$none);
			case 'MeasureFontRequested':
				return _Utils_Tuple2(
					model,
					A2(
						$elm$core$Task$attempt,
						$author$project$Main$MeasureFontReceived,
						A2(
							$elm$core$Task$map,
							function (_v1) {
							...
							$elm$browser$Browser$Dom$getElement('font-measure'))));
			case 'MeasureFontReceived':
				var measureResult = msg.a;
				if (measureResult.$ === 'Ok') {
					var measuredSize = measureResult.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
...
var $author$project$Main$inlineCss = A3(
	$elm$html$Html$node,
	'style',
	_List_Nil,
	_List_fromArray(
		[
			$elm$html$Html$text($author$project$Main$inlineRawCss)
		]));
...
var $author$project$Main$textareaStyle = function () {
	var style = F2(
		function (a, b) {
			return A3(
				$elm$html$Html$node,
	...
	return A2(style, '#f8f8f2', 'rgba(255,255,255,0.2)');
}();
...
var $author$project$Main$toHtml = F4(
...
var $author$project$Main$toHtmlPython = $author$project$Main$toHtml($pablohirafuji$elm_syntax_highlight$SyntaxHighlight$python);
...
var $author$project$Main$viewFontMeasure = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			$elm$html$Html$Attributes$id('font-measure')
		]),
	_List_fromArray(
		[
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$style,
							'transform',
							'translate(' + ($elm$core$String$fromInt(-model.languageModel.scroll.left) + ('px, ' + ($elm$core$String$fromInt(-model.languageModel.scroll.top) + 'px)')))),
			...
			[
				...
				_List_fromArray(
					[
						...
						A2($author$project$Main$viewLanguage, $author$project$Main$toHtmlPython, model)
					]))
			]),
...
var $author$project$Main$main = $elm$browser$Browser$document(
	{
		init: function (_v0) {
			return _Utils_Tuple2(
				...
				A2(
					$elm$core$Task$perform,
					$elm$core$Basics$always($author$project$Main$MeasureFontRequested),
			...
			$elm$core$Platform$Sub$batch(
				_List_fromArray(
					[
						...
						$elm$core$Basics$always($author$project$Main$Frame))
					]))),
